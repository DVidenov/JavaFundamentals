        ...
        while (!command.equals("Retire")){
            ...
            switch (commandName){
                case "Fire":
                    ...
                    break;
                case "Defend":
                    int startIndexes = Integer.parseInt(commandArguments[1]);
                    int endIndexes = Integer.parseInt(commandArguments[2]);
                    int damageDealt = Integer.parseInt(commandArguments[3]);
                    if (startIndexes < 0 || startIndexes >= pirateShip.size()) {
                        break;
                    }
                    if (endIndexes < 0 || endIndexes >= pirateShip.size()) {
                        break;
                    }
                    for (int i = startIndexes; i <= endIndexes; i++) {
                ...
                case "Repair":
                    ...
                    pirateShip.set(repairIndex, sectionToRepair); // pirateShip[repairIndex] = repairHealth;
                    break;
                case "Status":
                    int count = 0;
